...
def load_csvs():
    ...
    """
    df_samples = pd.read_csv('samples.csv', low_memory=False)
    df = pd.read_csv('pattern_counts.csv', low_memory=False)
    sample_info = pd.read_csv('sample_info.csv', low_memory=False)
    target_info = pd.read_csv('target_info.csv', low_memory=False).set_index('target')
    target_table = pd.read_csv('target_table.csv', low_memory=False).set_index('target')
    df_fcs = pd.read_csv('fcs_stats.csv', low_memory=False)
...
def calculate_pairwise(df, groupby='sgRNA', func=jaccard):
    """Group samples by 
    """
...
def plot_enrichments(df, gate_order=('none', 'GFP+'), yscale='log'):
    df_sgRNA = melt_sgRNA_libraries(df)
    df_sgRNA = df_sgRNA.query('library & ~(day=="d28" & gate=="none")')
    filt = df_sgRNA['gate'].isin(list(gate_order))
    ...
    fg = sns.factorplot(data=df_sgRNA[filt], hue='closest_sgRNA', x='gate', col='target', 
                   row='background', y='fraction', kind='bar', 
                        col_order=col_order, order=gate_order, hue_order=hue_order)
...
enrichment_singles = [('sgV-1K', 'T9')
                     ,('sgV-1K', 'T12')
                     ,('sgV-3K', 'T12')
                     ,('sgV-3K', 'T13')
                     ,('sgV-3K', 'T14')]

def plot_enrichments_single(df, singles, gate_order=('none', 'GFP+'), yscale='log'):
    """singles = ('sgV-1K', 'T11')
    """
    df_sgRNA = melt_sgRNA_libraries(df)
    df_sgRNA = df_sgRNA.query('library & ~(day=="d28" & gate=="none")').copy()
    filt = df_sgRNA['gate'].isin(list(gate_order))

    hue_order = 'other', 'sgV-101', 'sgV-106', 'sgV-107', 'sgV-116', 'sgV-117'
    df_sgRNA['plot'] = ''
    title = 'background = %s | target = %s'
    titles = []
    for background, target in singles:
        t = title % (background, target)
        filt2  = df_sgRNA['background'] == background
        filt2 &= df_sgRNA['target'] == target
        df_sgRNA.loc[filt2, 'plot'] = t 
        titles += [t]

    fg = sns.factorplot(data=df_sgRNA[filt], hue='closest_sgRNA', x='gate', col='plot', 
                    y='fraction', kind='bar', col_wrap=3,
                        col_order=titles, order=gate_order, hue_order=hue_order)
    ...
    for ax in fg.axes.flat[:]:
        ax.set_title(ax.get_title().replace('plot = ', ''))
